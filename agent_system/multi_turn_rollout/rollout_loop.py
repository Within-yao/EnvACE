...
class MultiAgentTrajectoryCollector(TrajectoryCollector):
    ...
    def __init__(
        ...
        elif orchestra_type == "checklist":
            from agent_system.agent.orchestra.checklist import ChecklistMultiAgentOrchestra as orchestra
        else:
    ...
    def vanilla_multi_turn_loop(
        ...
        for _step in range(self.config.env.max_steps):
            ...
            # optional checklist metadata (constant per-sample once set by env)
            num_checklists_arr = np.array(
                [int(info.get('num_checklists', 1)) for info in infos], dtype=np.int32
            )
            max_num_turns_arr = np.array(
                [int(info.get('max_num_turns', 1)) for info in infos], dtype=np.int32
            )
            for data in multiagent_batch_buffer:
                ...
                agent_batch.non_tensor_batch['num_checklists'] = num_checklists_arr.astype(object)
                agent_batch.non_tensor_batch['max_num_turns'] = max_num_turns_arr.astype(object)

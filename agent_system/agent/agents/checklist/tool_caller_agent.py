...
def _preprocess_checklist_batch(
    ...
    # Rollout-time prompt budget — must be set explicitly. No fallback to
    # data.max_prompt_length (see tool_simulator_agent.py for rationale).
    ...
    for i in range(batch_size):
        ...
        if tools_list is not None and i < len(tools_list):
            ...
            # tools are carried in gen_batch.non_tensor_batch['tools'] from dataset
...
# ============ X.2: caller-side incremental preprocess ============
def _preprocess_checklist_batch_incremental(
    ...
    # Per-sample truncation (left-pad later)
    ...
    for i in range(B):
        ...
        if ids_i is None:
            # Dummy/padding slot — create zero-length tensors. _generate_with_llm
            # should mask these out via agent_active_mask anyway.
        ...
        # X.2 step 12: env should have terminated active episodes that exceed budget
        # via _check_caller_length_or_terminate (mirrors upstream sglang_rollout.py:976).
        # 但 inactive sample（state ∈ AWAIT_SIMULATOR / PROCESSING_TOOLS / INTERACTING
        # 等中间态）允许 cur_len 暂时上浮 —— env 等转回 AWAIT_CALLER 时才查；这一步
        # 它们的输出会被 active mask 屏蔽，左截断到 budget 仅作占位即可。
    ...
    # Left-pad to GLOBAL rollout_max_prompt_length so per-turn records have
    # uniform shape across turns (matches full-render path & upstream's
    # fixed-length-per-trajectory invariant). Required so downstream
    # collate_fn can torch.stack samples from different turns without
    # triggering variable-shape padding (which would corrupt input_ids
    # with token id=0, decoded literally as "!" by the Qwen tokenizer).
    ...
    for i in range(B):
        ...
        raw_prompt_ids = ids_i[0].tolist()  # un-padded list for sglang skip_tokenizer_init
        ...
        rows.append({
            ...
            "index": i,
            # caller_loss_mask is NOT carried into DataProto here. Current PPO mode A
            # (turn-level slicing) doesn't read it, so propagating it would (a) be dead
            # weight in the buffer and (b) cause multi-agent collate to assert because
            # other agents (e.g. tool_simulator_agent) don't produce this key. The env
            # layer (envs.py:612, 741) still maintains the token-level mask in the
            # ChecklistEnv state object so mode B (episode-level PPO) can be re-enabled
            # by (1) re-adding `obs["caller_loss_mask"] = ...` in env_manager.py:263 and
            # (2) re-adding the read/pad/write of this field here.
        })
...
class ToolCallerAgent(BaseAgent):
    ...
    def call(
        ...
        # X.2: switch to incremental path when env exposes caller token state
        # (use_caller_incremental=True). Otherwise fall back to full-render path.
        use_incremental = (
        ...
        # Validity = any caller action decodes (we don't require <tool_call>; final-answer turns are valid)

...
# ---------------------------------------------------------------------------
# Output-contract constants — 1:1 mirror caller-side `tool_parser.py`
# (only the tag name is swapped: <tool_call> → <execution_result>).
# ---------------------------------------------------------------------------
# NOTE: the simulator always uses VALIDATION_SIMULATOR_SYSTEM_INSTRUCTION.

EXECUTION_RESULT_START = "<execution_result>"            # cf. tool_parser.TOOL_CALL_START
EXECUTION_RESULT_BOT = "\n<execution_result>\n"          # cf. tool_parser.BOT_TOKEN
EXECUTION_RESULT_EOT = "\n</execution_result>"           # cf. tool_parser.EOT_TOKEN
...
def _parse_sim_response(text: str) -> str:
    ...
    # ---- Phase 0: strip <think>...</think> blocks (do NOT .strip() the result —
    # that would eat the leading \n required by the EXECUTION_RESULT_BOT match,
    # mirroring caller-side tool_parser which also leaves whitespace intact). ----
    if text:
        text = _THINK_BLOCK_RE.sub("", text)
        # only strip whitespace at very start/end if there is no <execution_result> block
        # straddling those boundaries (we want to preserve the \n before BOT)
        if not text.strip():
            text = ""

    # ---- Phase 1: substring detection (cf. tool_parser:51 'TOOL_CALL_START in text') ----
    if not text:
    ...
    # ---- Phase 2: regex extract <execution_result>...</execution_result> block ----
    matches = _EXECUTION_RESULT_RE.findall(text)
    if not matches:
        # tag substring present but \n boundaries / closing tag missing
        # (mirrors toolcall.md scenario 2/6 — caller's NO_TOOL_CALL_FOUND case)
        return format_tool_error(
    ...
    # Multiple blocks → take the first (graceful; early simulator may emit duplicates)
    block = matches[0].strip()

    # ---- Phase 3: JSON decode (cf. tool_parser:74-82) ----
    try:
    ...
    # ---- Phase 4+5: type constraint, byte-aligned with upstream response_format json_schema:
    #   anyOf: [
    ...
    if isinstance(parsed, dict):
        ...
        pass  # any list (incl. empty) accepted, mirroring upstream schema items:{}
    else:
        return format_tool_error(
            code="INVALID_EXECUTION_RESULT_TYPE",
            message="execution_result must be a non-empty object or an array",
    ...
    # ---- Phase 6: serializable (cf. tool_parser:104-114) ----
    try:
    ...
    # ---- Phase 7 removed (length truncation): aligned with upstream sglang_rollout
    # which does NOT truncate single tool responses. Return verbatim.
    return er_str
...
class ToolSimulatorAgent(BaseAgent):
    def __init__(self, wg_id: str, tokenizer: PreTrainedTokenizer, processor, config: Any):
        ...
        # Length-truncation removed: aligned with upstream sglang_rollout.py path
        # (rollout.mode=sync, traj_n48.sh:91) which does NOT truncate single
        # tool responses. Caller-side rollout_max_prompt_length=12000
        # left-truncation is the global safety net. The previous
        # `simulator_response_max_len` / `simulator_response_truncate_side`
        # config fields are accepted but ignored for backward compatibility.
    ...
    def call(
        ...
        # upstream-style: rollout-time tokenize uses a larger budget than the
        # dataset-level initial filter when rollout_max_prompt_length is set.
        ...
        for i in range(B):
            ...
            if active and isinstance(payload, dict):
                ...
                # Dummy (inactive): minimal prompt so tokenizer/chat_template still works.
                chat = [{"role": "system", "content": "."}, {"role": "user", "content": "."}]

            # apply_chat_template walks the same path as caller — Qwen3 default
            # enable_thinking=True is preserved (think mode on, plan §10.(f)).
            ...
            rows.append({
                ...
                "raw_prompt": chat,
                "anchor_obs": [],  # simulator no longer feeds full conversation
        ...
        # try/except around _generate_with_llm — mirrors upstream tool_agent_loop._call_tool
        # outer wrapper. Rollout backend failures fall back to a flat-string error
        # sentinel; trajectory does NOT hard-terminate (matches upstream: judge handles
        # via low score). See plan §10.(b).
        try:
        ...
        # Single-string output. Strict-parse:
        #   success → result JSON string
        #   failure → format_tool_error(...) JSON; envs.py:265-280 detects
        #             `error_tool_call` key → sim_failed_any=True → terminate.
        # If the rollout exception path produced a flat-string sentinel
        # ("Error when executing tool: ...") — pass it through as-is; envs.py
        # will not detect `error_tool_call` and trajectory continues
        # (matches upstream _call_tool: judge gives low score).
        outputs: List[str] = [""] * B
        for i in range(B):
            ...
            if raw.startswith("Error when executing tool:"):
                outputs[i] = raw  # rollout-failure sentinel; pass through
            else:

...
class ChecklistMultiAgentOrchestra(BaseOrchestra):
    ...
    def __init__(
        ...
        # Trigger @AgentRegistry.register side-effects for checklist agents
    ...
    def run(
        ...
        if need_agent is None:
            # First tick after reset — everyone awaits Caller
            need_agent = ["caller"] * B
        ...
        if simulator_mask.any():
            if not self.has_simulator:
                # Env expects a simulator reply but we have no Simulator agent registered.
                # Fill with empty strings; env will treat as empty tool responses.
                for i in np.where(simulator_mask)[0]:
                    text_actions[i] = "[]"
            else:
                sim_wg = actor_rollout_wgs[self.agents_to_wg_mapping[self.SIMULATOR]]
                # Delegate to Simulator agent; its call() handles the per-sample tool-call fan-out
                # and returns a JSON-encoded list[str] per sample aligned with simulator_payload.

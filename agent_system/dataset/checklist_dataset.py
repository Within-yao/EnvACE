...
class ChecklistDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # ---- EnvACE integration ----
        # Package everything the ChecklistEnv needs into a single env_kwargs dict.
        # rollout_loop will pop this from gen_batch.non_tensor_batch and pass list[env_kwargs]
        # to envs.reset(kwargs=...).
        row_dict["env_kwargs"] = {
            "messages": messages,
            "tools": tools,
            "checklist_list": interaction_kwargs["checklist_list"],
            "all_messages": interaction_kwargs["all_messages"],
            "uuid": str(original_index),
            "data_source": row_dict.get("data_source", "checklist"),
        }
        # EnvACE reward manager reads data_source from non_tensor_batch
        row_dict.setdefault("data_source", "checklist")
        return row_dict

    def __getstate__(self):
        # Drop the materialized dataframe from the pickle payload; it is rebuilt
        # from the parquet files by _read_files_and_tokenize on the other side.

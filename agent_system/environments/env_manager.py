...
class ChecklistEnvironmentManager(EnvironmentManagerBase):
    ...
    def __init__(self, envs, projection_f, config):
        super().__init__(envs, projection_f, config)
        # Cache last step's tools/needs so Orchestra can route properly.
        self._last_tools: List[Any] = []

    def reset(self, kwargs) -> Tuple[Dict[str, Any], List[Dict]]:
        # Save tool schemas per sample (stable across the episode) for chat_template.
        # (kwargs may be a numpy object-array from DataProto; avoid `or`/truthy on ndarray)
        self._last_tools = []
    ...
    def _build_obs(self, messages_list: List[List[Dict]], infos: List[Dict]) -> Dict[str, Any]:
        B = len(messages_list)
        # Router hints (per-sample) for ChecklistOrchestra
        need_agent = [info.get("need_agent", "caller") for info in infos]
        sim_payload = [info.get("simulator_payload") for info in infos]
        # tools were provided at reset; if batch sizes don't match (padding etc.), fall back to []
        ...
        if len(tools) < B:
            ...
            tools = tools[:B]
        obs = {
            "text": None,  # ToolCallerAgent uses 'anchor' directly
            "image": None,
        ...
        # X.2 incremental: expose caller-side token state when use_caller_incremental=True.
        # ToolCallerAgent._preprocess_checklist_batch_incremental reads these instead of
        # re-rendering messages via apply_chat_template.
        # NOTE: the env layer also maintains caller_loss_mask for a future mode B
        # (episode-level PPO), but it's intentionally NOT exposed here — current PPO
        # mode A doesn't read it, and exposing it would carry dead weight all the way
        # to DataProto where multi-agent collate would assert. To enable mode B, widen
        # ChecklistMultiProcessEnv.get_caller_token_state to return it as well, then
        # add the corresponding read/pad/write in tool_caller_agent.py.
        if getattr(self.config.env.checklist, "use_caller_incremental", False):
            # Slots beyond B are batch padding; the env keeps them for shape stability
            # but they carry no real sample, so trim to the live batch here.

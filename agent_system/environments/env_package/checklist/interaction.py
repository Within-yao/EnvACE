...
class ChecklistInteraction:
    """Checklist interaction (adapted from upstream).
    ...
    def __init__(self, config: dict):
        # Note: BaseInteraction.__init__ stored `self.config = config`, replicate that
        self.config = config
        ...
        # Judge backend: "sglang" (default, local) | "openai" (remote API)
        self._judge_backend = config.get("judge_backend", "sglang")
        self._judge_api_key = config.get("judge_api_key", None)
        ...
        headers = {}
        if self._judge_backend == "openai":
            if not self._judge_api_key:
                raise ValueError(
                    "ChecklistInteraction: judge_backend='openai' requires judge_api_key"
                )
    ...
    async def generate_response(
        ...
        # 4th tuple slot is per_step_api_failed (added by 3-tuple eval_one_check). Backward compat:
        # if the inner coroutine returned only 3 tuple items, fallback to all-False api_failed.
        per_step_api_failed_list = [
            result[3] if len(result) > 3 else [[False] * len(per_step_call_success_list[i][s])
                                                 for s in range(len(per_step_call_success_list[i]))]
        ...
        # Distinguish two failure modes (was conflated as `call_success=False`):
        #   (a) api_failed=True   → remote 502/timeout/connect error.
        #   (b) api_failed=False, call_success=False → judge returned malformed JSON.
        # Either way the sequence is terminated so the model sees the error feedback
        # (upstream-style learning signal).
        api_failed_count = 0
        judge_malformed_count = 0
        for x_call, x_api in zip(per_step_call_success_list, per_step_api_failed_list):
            for y_call, y_api in zip(x_call, x_api):
                for z_call, z_api in zip(y_call, y_api):
                    if z_api:
                        api_failed_count += 1
                    elif z_call is False:
                        judge_malformed_count += 1
        if api_failed_count > 0 or judge_malformed_count > 0:
            logger.warning(
                f"reward gen failed (api_failed={api_failed_count}, judge_malformed={judge_malformed_count}), terminate seq"
            )
            should_terminate_sequence = True
    ...
    async def generate_response_for_single_checklist(
            ...
            if messages[i]["role"] == "assistant":
                ...
                for single_step_checklist in this_turn_checklist:
                    ...
                    async def _guarded_eval_tool_error() -> bool:
                            return False, True, False  # (judge=False, call_success=True, api_failed=False) — tool failed isn't an API failure
        ...
        org_flat_per_step_results: list[tuple[bool, bool, bool]] = await asyncio.gather(*all_step_results_this_turn) # a list of bool lenght is steps * len(this_turn_checklist), the first len(this_turn_checklist) is for step 0
        ...
        # api_failed bit: True iff the API call itself raised (502/timeout). Distinguishes
        # "remote infra failure" from "judge returned malformed JSON" or "judge legitimately
        # returned False". Used downstream to MASK the trajectory from GRPO advantage so
        # API outages don't pollute training gradients with fake `score=0`.
        flat_per_step_api_failed = [bool(x[2]) if len(x) > 2 else False for x in org_flat_per_step_results]
        ...
        per_step_api_failed = [flat_per_step_api_failed[i:i+len(this_turn_checklist)] for i in range(0, len(flat_per_step_api_failed), len(this_turn_checklist))]
        ...
        return should_terminate_sequence, per_step_results, per_step_call_success, per_step_api_failed

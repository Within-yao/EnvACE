...
# Qwen3 soft-switch toggle — when enabled, append "\n\n/think" at end of sim user
# prompt to force thinking-mode (Mode A) on Qwen3 hybrid models. Empirically
# (z_design §III.4.6.15) this drops empty-think rate from ~45% to ~7.5% on
# Qwen3-8B base. Default ON; set SIM_APPEND_THINK_SWITCH=false to disable.
...
def build_validation_user_prompt(
    ...
    *,
    ...
    # Qwen3 soft-switch /think — append at end to force thinking mode.
    # Default ON; disable via SIM_APPEND_THINK_SWITCH=false.
    # See z_design §III.4.6.15 for the 10-version A/B experiment that confirmed
    # /think drops empty-think rate from ~45% to ~7.5% on Qwen3-8B base.

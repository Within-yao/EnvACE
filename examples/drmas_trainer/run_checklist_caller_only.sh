...
#
# Trains the Tool Caller Agent alone. The Tool Simulator Agent is not
# instantiated; tool responses are served by a remote OpenAI-compatible
# endpoint (TOOL_API_URL) instead.
#
...
rollout_max_prompt_length=12000
...
TOOL_API_URL=${TOOL_API_URL:?need TOOL_API_URL=<endpoint>/v1/chat/completions when TOOL_RETURN_MODE=httpx}

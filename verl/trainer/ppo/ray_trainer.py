...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # Resolve frozen_agent_ids → frozen wg_ids. Frozen agents still do rollout
        # and contribute to advantage / reward, but skip the actor gradient update.
        frozen_ids = set(self.config.agent.get("frozen_agent_ids", []) or [])
        self.frozen_wg_ids = {
            wg_id for wg_id, agents in self.wg_to_agents_mapping.items()
            if any(a["agent_id"] in frozen_ids for a in agents)
        }
        if self.frozen_wg_ids:
            print(f"[ray_trainer] Frozen wg_ids (no gradient update): {self.frozen_wg_ids}")
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with _timer("update_actor", timing_raw):
                            for wg_id in multiagent_batch.keys():
                                ...
                                if wg_id in self.frozen_wg_ids:
                                    # Frozen agent: rollout already done, skip gradient update.
                                    multiagent_batch[wg_id] = sub_batch
                                    continue
